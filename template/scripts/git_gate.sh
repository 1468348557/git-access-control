...
check_diff_size() {
  ...
  if [[ "${DIFF_TOTAL_LINES}" -gt "${AUTO_MERGE_MAX_LINES}" ]]; then
    echo "[WARN] MR 变更超过${AUTO_MERGE_MAX_LINES}行（${DIFF_TOTAL_LINES}行），需要人工审批"
  else
    pass "MR 变更行数校验通过"
  fi
}
...
main() {
  ...
  if [[ "${PIPELINE_SOURCE}" == "merge_request_event" ]]; then
    ...
    # 变更不超过阈值时直接调 API 自动合并
    if [[ "${DIFF_TOTAL_LINES}" -le "${AUTO_MERGE_MAX_LINES}" ]]; then
      echo "门禁通过，执行自动合并..."
